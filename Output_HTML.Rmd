...
output:
  word_document:
    fig_caption: yes
    reference_docx: //deqhq1/TMDL/AgWQM/DataAnalysis/StatusAndTrendsRMarkdown/ReportTemplate.docx
    toc: yes
  html_document:
    mode: selfcontained
    number_sections: yes
...
if(any(df.all$Analyte == 'Enterococcus')){
  ...
  if(any(status$Analyte == 'Enterococcus')){
    ent_stat<-status %>%
      dplyr::filter(Analyte == "Enterococcus")
    ent_stat <- c(unique(e_stat$Station_ID))
  } else {ent_stat <- NULL}
  
  if(any(trend$Analyte == 'Enterococcus')) {
    ent_stns<-trend %>%
      dplyr::filter(Analyte == "Enterococcus")
    ent_stns<-c(unique(ent_stns$Station_ID))
  } else {ent_stns <- NULL}
  
  ent_stns <- unique(append(ent_stns, ent_stat))
  ent_list<- list()
  exc_list<- list()
  ...
  if(length(ent_stns > 0)){
    ...
    kable(exc, padding = 2, digits = 1, caption = tbls(name = "enterExcTable", caption = "Enterococcus status and trends, if sufficient data exists to calculate the geometric mean, it is included in the table.**"))
