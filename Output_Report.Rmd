---
title: '<img src="LogoColorRegular.jpg" style="float: right;width: 80px; "/> Oregon DEQ Water Quality Status and Trends report for the Umpqua River AgWQ Management Area'
date: April 2018
output:
  word_document:
    fig_caption: yes
...
project_dir <- "//deqhq1/WQNPS/Agriculture/Status_and_Trend_Analysis/Umpqua River/2018/StatusAndTrendsRMarkdown"
...
if(file.exists(file=paste0(Rdata_dir,"/",df_all_file))) {
  
  print("loading existing df.all")
  load(file=paste0(Rdata_dir,"/",df_all_file))
  ...
  save(df.all, file=paste0(Rdata_dir,"/",df_all_file))
}
...
df.all <-df.all[df.all$Station_ID %in% Stations_in_poly(df.all, agwqma_shp[agwqma_shp$PlanName==input$select,]),]
...
![](Figures/Agreview_flowchart_V5.png)
...
agarea <- agwqma_shp[agwqma_shp$PlanName == input$select,]
...
```{r Ecoli, results = 'asis', fig.width = 6, fig.height = 6}
...
stns_param_summary <-  parm_summary(stns = stns[,c(1:5)],
                            ecoli = ecoli,
                            pH = pH,
                            DO_eval = DO_eval,
                            temp = temp,
...
# Reconcile different Datums before making a shapefile
# Datums to search for
# NAD83 : EPSG:4269 <- This is assumed if it is not one of the other two
# NAD27 : EPSG:4267
# WGS84 : EPSG:4326

df.nad83 <- stns_param_summary_shp[!grepl("NAD27|4267|WGS84|4326",toupper(stns_param_summary_shp$DATUM)), ]
df.nad27 <- stns_param_summary_shp[grepl("NAD27|4267",toupper(stns_param_summary_shp$DATUM)), ]
df.wgs84 <- stns_param_summary_shp[grepl("WGS84|4326",toupper(stns_param_summary_shp$DATUM)), ]
...
# convert to NAD 83
if(NROW(df.nad27) > 0) {
   df.nad83 <- rbind(df.nad83, spTransform(df.nad27, CRS("+init=epsg:4269")))
}

if(NROW(df.wgs84) > 0 ) {
  df.nad83 <- rbind(df.nad83, spTransform(df.wgs84, CRS("+init=epsg:4269")))
  }

# combine back into single df
stns_param_summary_shp <- df.nad83

rm(df.nad83, df.nad27, df.wgs84)
...
stns_param_summary <- stns_param_summary %>%
                  dplyr::select(-DATUM)
...
knitr::kable(stns_param_summary, padding = 2, digits = 3, caption = tbls(name = "parmSumTable", caption = "Summary of Monitoring Stations Status and Trend, where 'exceed' represents a single exceedance of the water quality standard within the last two years of available data. Note: DO = dissolved oxygen**"))

# map2 <- leaflet(agarea) %>%
#   addPolygons() %>%
#   addTiles() %>%
#   #addRasterImage(lulc) 
#   addMarkers(lng = stns_param_summary$DECIMAL_LONG, 
#              lat = stns_param_summary$DECIMAL_LAT, 
#              popup = paste(stns_param_summary$Station_ID, "-", stns_param_summary$Status_bacteria, stns_param_summary$Trend_bacteria, sep = ",")) %>%
#             addWMSTiles(GetURL("USGSTopo"), 
#                         attribution = paste0("<a href='https://www.usgs.gov/'>",
#                                              "U.S. Geological Survey</a> | ",
#                                         "<ahref='https://www.usgs.gov/laws/policies_notices.html'>",
