...
The available data was not sufficient to determine any status or trends for total phosphorus.
...
if(length(pH_stns) > 0) {
  ...
  df.pH <- df.all %>% filter(Analyte == 'pH', Station_ID %in% pH_stns)
  ...
  df.pH <- merge(df.pH,in_bentable, by.x = "Station_ID", by.y = "Station_ID", all.x = TRUE)
...
knitr::kable(stns_param_summary_tbl[,c(1:4,11:16)],
             ...
             caption = tbls(name = "parmSumTable2", 
                            caption = paste0("Continued summary of monitoring stations status and trend, where 'Exceeds' represents a single exceedance of the water quality standard or TMDL target within the last two whole years. See ", tbls("parmSumTable1", cite = ), " Note: DO = dissolved oxygen, TP = total phosphorus, TSS = total suspended solids.")))
# 
