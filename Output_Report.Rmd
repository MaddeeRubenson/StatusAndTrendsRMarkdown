...
if(file.exists(file=paste0(Rdata_dir,"/",df_all_clean_file))) {
  ...
  # BTD Remove enterococci data outside non-freshwater polygon
  
  # Create a subset of data only including "Enterococcus"
  df.temp.ent <- subset(df.all, Analyte == 'Enterococcus')
  
  # Get stations inside bacteria_nonfreshwater polygon  
  df.all <- df.all[!(df.all$Station_ID %in% Stations_in_poly(df.temp.ent, bacteria_nonfreshwater_shp, outside=TRUE) &            df.all$Analyte == 'Enterococcus'),]
