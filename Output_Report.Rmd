---
title: '<img src="Figures/LogoColorRegular.jpg" style="float: right;width: 80px; "/>Upper Grande Ronde Agricultural Water Quality Management Area Water Quality Status and Trends Report'
...
agwqma <- "Upper Grande Ronde"
query_dates <- c("2000-01-01", "2018-12-01")
project_dir <-  "//deqhq1/WQNPS/Agriculture/Status_and_Trend_Analysis/Upper Grande Ronde/2018-Upper-Grande-Ronde"
...
if(file.exists(file=paste0(Rdata_dir,"/",df_all_clean_file))) {
  ...
  # Combine McCoy stations 1:5
  df.all[df.all$Station_ID %in% c("5123", "5124", "5125", "5126"),"Station_ID"] <- "5122"
  df.all[df.all$Station_ID =="5122", "Station_Description"] <- "McCoy Stations 1-5"
  df.all[df.all$Station_ID =="5122", "DECIMAL_LAT"] <- 45.26371
  df.all[df.all$Station_ID =="5122", "DECIMAL_LONG"] <- -118.4014
  ...
  if (any('Temperature' %in% df.all$Analyte)) {
    ...
    if(NROW(qc.3.pass) > 0) {
      ...
      df.all <- merge(df.all, qc.3.pass[,c("Station_ID", "month", "result", "Analyte")], 
...
Five stations along the McCoy River were representative of the same reach. For this report, data from stations 5122, 5123, 5124, 5125, and 5126 were combined under station ID 5122 to simplify analysis.
...
knitr::kable(stns_param_summary_tbl[,c(1:2,5:10)],
             padding = 2, digits = 3,
             row.names = FALSE,
             caption = tbls(name = "parmSumTable1", 
                            caption = "Summary of monitoring stations status and trend, where 'Exceeds' represents a single exceedance of the water quality standard or TMDL target within the last two whole years."))
cat("\n\n")
knitr::kable(stns_param_summary_tbl[,c(1:4,11:16)],
             padding = 2, digits = 3,
             row.names = FALSE,
             caption = tbls(name = "parmSumTable2", 
                            caption = paste0("Continued summary of monitoring stations status and trend, where 'Exceeds' represents a single exceedance of the water quality standard or TMDL target within the last two whole years. See Table ", tbls("parmSumTable1", display = 'num'), " Note: DO = dissolved oxygen, TSS = total suspended solids, TP = total phosphorus.")))
...
# Add additional conclusions in the form of conclusions.add.txt <- c("Additional conclusion 1", "Additional conclusion 2", etc)
conclusions.add.txt <- NULL
...
if(!is.null(conclusions.add.txt)){
...
```{r add-conclusions, eval = add_conc}
cat(paste0("**Additional Conclusions:** \n\n"))

cat(paste0("* ",conclusions.add.txt,"\n", collapse=""))
```
