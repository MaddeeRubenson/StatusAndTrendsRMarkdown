...
landuse_map_file <- paste0("//deqhq1/WQNPS/Agriculture/Status_and_Trend_Analysis/Molalla-Pudding French Prairie North Santiam/2018/StatusAndTrendsRMarkdown/Figures/LandUseMap_MPFPNS.png")
station_map_file <- paste0("//deqhq1/WQNPS/Agriculture/Status_and_Trend_Analysis/Molalla-Pudding French Prairie North Santiam/2018/StatusAndTrendsRMarkdown/Figures/Station_Locations_MPFPNS.png")
station_summary_map_file <- paste0("//deqhq1/WQNPS/Agriculture/Status_and_Trend_Analysis/Molalla-Pudding French Prairie North Santiam/2018/StatusAndTrendsRMarkdown/Figures/Station_Param_Summary_MPFPNS.png")
...
wq_limited <- read.csv('Lookups/WQL_MPFPNS.csv')
...
deq<- paste(BC_LU[BC_LU$AgArea %in% input$select, 'DEQ_bc_name'][1], BC_LU[BC_LU$AgArea %in% input$select, 'DEQ_bc_email'][1], sep = '; ')
deq2 <- paste(BC_LU[BC_LU$AgArea %in% input$select, 'DEQ_bc_name'][2], BC_LU[BC_LU$AgArea %in% input$select, 'DEQ_bc_email'][2], sep = '; ')
...
BC_table<-data.frame(input$select, deq, deq2, oda)
colnames(BC_table) <- c('AgWQ Management Area', 'DEQ Basin Coordinator', 'DEQ Basin Coordinator', 'ODA AgWQ Specialist')

knitr::kable(BC_table[1,], 
             # format = "markdown", 
...
`r tbls(name = "resutltsTable", caption = paste0("Monitoring stations with sufficient data to assess status and/or trends within the ", agwqma, " AgWQ Management Area.**"))`
...
```{r landuse-map-word, eval = word_output, results = "asis", fig.cap=figs(name="landusemap", caption="Land use and land cover within the North Coast AgWQ Management Area**")}

knitr::include_graphics(path = paste0(landuse_map_file))

```


```{r landuse-map-html, results= 'asis', eval = !(word_output), fig.width=10, fig.height=8}
...
landuse <-landuse[order(landuse$`Station ID`), - 3]

save(landuse, file=paste0(Rdata_dir,"/", landuse_file))

knitr::kable(landuse, digits = 0, 
             # format = "markdown", 
             padding = 2, caption = tbls(name = "landUse", caption = "Summary table of watershed land use by station, only stations which have at least 8 years of yearly data (between 2000 and 2017) and/or are used to evaluate last known status. Source: 2011 NAIP**"))
...
For all monitoring stations with sufficient data (8 years or more), trending was performed using Seasonal Kendall trend analysis. Results are summarized in Table `r tbls("seakenTable", display = "cite")` (If no table is visible, there were no monitoring station that contained sufficient data to assess trends).  Sections 4.4. to 4.11 show the trend line on each of the figures for the stations where a significance level can be established. Sections 4.4 to 4.11 also plot the water quality status when applicable at the monitoring location.
...
if(length(ent_stns) > 0) {
  ...
  cat("No monitoring stations have data to assess status and/or trend of Enterococcus") 
} 
...
exc <- NULL
tss_stn <- NULL
pudding_stns <- unique(df.all[grepl("Pudding River", df.all$Station_Description) & !grepl("Little Pudding River", df.all$Station_Description),"Station_ID"])
zollner_stns <- unique(df.all[grepl("Zollner Creek", df.all$Station_Description),"Station_ID"])
littlePudding_stns <- unique(df.all[grepl("Little Pudding River", df.all$Station_Description),"Station_ID"])
...
if(!(trend == "No Stations Meet Trend Criteria")) {
  ...
  for(j in 1: length(tss_stn)) {
    ...
    if (tss_stn[j] %in% c(pudding_stns, zollner_stns)) {
    TSS_evaluate <- EvaluateTSSWQS(tmp_df, 
                                   selectWQSTSS = 15)
    } else if (tss_stn[j] %in% littlePudding_stns) {
    TSS_evaluate <- EvaluateTSSWQS(tmp_df, 
                                   selectWQSTSS = 7)
    } else {
      TSS_evaluate <- EvaluateTSSWQS(tmp_df, 
  ...
  for (i in 1:length(tss_stn)) {
    ...
    if (tss_stn[i] %in% c(pudding_stns, zollner_stns)) {
      b <- plot.TSS(new_data=mydata_sub,
                  ...
                  selectWQSTSS = 15,
                  parm = unique(mydata_sub$Analyte))
    } else if (tss_stn[i] %in% littlePudding_stns) {
      b <- plot.TSS(new_data=mydata_sub,
                  df.all = df.all,
                  sea_ken_table=results_seaken ,
                  plot_trend = trend_logic,
...
```{r Ecoli-summary-map, eval = word_output, results = "asis", fig.cap=figs(name="Ecoli_map", caption="Summary of stations that fit the criteria for status and/or trend analysis. One or more exceedances within the last three years of available data defined whether a station was Meeting or Not Meeting. Trend was determined by significant trends associated with long-term datasets. Clusters of stations (grey circles located on the western margin of the study area) had insufficient data to determine status and trends.**")}

knitr::include_graphics(path = paste0(station_summary_map_file))
...
knitr::kable(stns_param_summary, padding = 2, digits = 3, caption = tbls(name = "parmSumTable", 
...
trendIcon <- function(trendColumn) {
  sapply(trendColumn, function(x){
    if(x %in% c("Improving", "Increasing")){
      "glyphicon-arrow-up"
    } else if(x %in% c("Degrading", "Decreasing")){
                    ...
                    lapply(rownames(stns_param_summary), 
                           function(row) {
                             htmlTable::htmlTable(stns_param_summary[row,c("Station ID", "Station Description",
                                                                           paste(param, "Status"), paste(param, "Trend"))],
...
# leaflet(agarea_shp) %>%
  ...
  addAwesomeMarkers(data = stns_param_summary,
                    ...
                    lng = stns_param_summary$Longitude,
                    icon = awesomeIcons(icon = trendIcon(stns_param_summary$`Ecoli Trend`),
                                        iconColor = 'black',
                                        library = 'glyphicon',
                                        markerColor = statusColor(stns_param_summary$`Ecoli Status`)),
  ...
  addAwesomeMarkers(data = stns_param_summary,
                    lat = stns_param_summary$Latitude,
                    lng = stns_param_summary$Longitude,
                    icon = awesomeIcons(icon = trendIcon(stns_param_summary$`Temperature Trend`),
                                        iconColor = 'black',
                                        library = 'glyphicon',
                                        markerColor = statusColor(stns_param_summary$`Temperature Status`)),
                    popup = popupTable("Temperature"),
                    group = "Temperature"
  ) %>% 
  ...
  addAwesomeMarkers(data = stns_param_summary,
                    lat = stns_param_summary$Latitude,
                    lng = stns_param_summary$Longitude,
                    icon = awesomeIcons(icon = trendIcon(stns_param_summary$`pH Trend`),
                                        iconColor = 'black',
                                        library = 'glyphicon',
                                        markerColor = statusColor(stns_param_summary$`pH Status`)),
                    popup = popupTable("pH"),
                    group = "pH"
  ) %>% 
  ...
  addAwesomeMarkers(data = stns_param_summary,
                    lat = stns_param_summary$Latitude,
                    lng = stns_param_summary$Longitude,
                    icon = awesomeIcons(icon = trendIcon(stns_param_summary$`DO Trend`),
                                        iconColor = 'black',
                                        library = 'glyphicon',
                                        markerColor = statusColor(stns_param_summary$`DO Status`)),
                    popup = popupTable("DO"),
...
knitr::kable(org_summary, 
             # format = "markdown", 
             padding = 2, digits = 1, caption = tbls(name = "orgSumTable", caption = "Summary table of all unique organizations that were queried; note that organizations included in this table may or may not have had data sufficient for status and/or trends analysis and therefore may not be included in this report**"))

knitr::kable(status, 
             # format = "markdown", 
             padding = 2, digits = 1, caption = tbls(name = "statusTable", caption = "Monitoring stations that fit the criteria to assess status; note value represents the number of results for each monitoring station per year**"))

knitr::kable(trend, 
             # format = "markdown", 
             padding = 1, digits = 1, caption = tbls(name = "trendTable", caption = "Monitoring stations that fit the criteria to assess trends; note value represents the number of results for each monitoring station per year**"))

knitr::kable(stns_by_year, 
             # format = "markdown", 
             padding = 1, digits = 1, caption = tbls(name = "stnByYrTable", caption = "All monitoring stations that were queried and the number of results per year from 2000 to 2017**"))
