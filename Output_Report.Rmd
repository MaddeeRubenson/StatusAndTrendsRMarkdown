---
title: '<img src="Figures/LogoColorRegular.jpg" style="float: right;width: 80px; "/>Lower North Fork Malheur Strategic Implementation Area Water Quality Status and Trends Report'
...
# sia, query_dates, and project_dir are different for every Report. Note 'sia' must be named exactly 
# how it is named in the sia shapefile ('agwqma_all_shp').

sia <- "Lower North Fork Malheur"
sia_huc8 <- c(17050116)

query_dates <- c("2000-01-01", "2018-12-31")
project_dir <-  "//deqhq1/WQNPS/Agriculture/Status_and_Trend_Analysis/SIAs/2019-SIA-Lower-North-Fork-Malheur"
...
landuse_map_file <- paste0(gsub(" ", "_", sia), "_landuse_map.png")
station_map_file <- paste0(gsub(" ", "_", sia), "_station_map.png")

station_summary_map1_file <- paste0(gsub(" ", "_", sia), "_station_summary_map1.png")
station_summary_map2_file <- paste0(gsub(" ", "_", sia), "_station_summary_map2.png")

station_summary_map_DO_file <- paste0(gsub(" ", "_", sia), "_station_summary_map_DO.png")
station_summary_map_pH_file <- paste0(gsub(" ", "_", sia), "_station_summary_map_pH.png")
station_summary_map_temp_file <- paste0(gsub(" ", "_", sia), "_station_summary_map_temp.png")
station_summary_map_ecoli_file <- paste0(gsub(" ", "_", sia), "_station_summary_map_ecoli.png")
station_summary_map_entero_file <- paste0(gsub(" ", "_", sia), "_station_summary_map_entero.png")
station_summary_map_tss_file <- paste0(gsub(" ", "_", sia), "_station_summary_map_tss.png")
station_summary_map_tp_file <- paste0(gsub(" ", "_", sia), "_station_summary_map_tp.png")

# Name of output files
df_all_raw_file <- paste0(gsub(" ", "_", sia), "_df_all_raw_",paste(query_dates, collapse = "."), ".Rdata")
df_all_clean_file <- paste0(gsub(" ", "_", sia), "_df_all_clean_",paste(query_dates, collapse = "."), ".Rdata")
duplicates_file <- paste0(gsub(" ", "_", sia), "_duplicates_",paste(query_dates, collapse = "."), ".Rdata")
landuse_file <- paste0(gsub(" ", "_", sia), "_landuse.Rdata")
stns_file <- paste0(gsub(" ", "_", sia), "_stns.Rdata")
status_file <- paste0(gsub(" ", "_", sia), "_status.Rdata")
trend_file <- paste0(gsub(" ", "_", sia), "_trend.Rdata")
stns_param_summary_file <- paste0(gsub(" ", "_", sia), "_station_param_summary.Rdata")
...
# import GIS features
# SIA area shp
sia_shp <- readOGR(dsn = GIS_dir, layer = 'LowerNorthForkMalheur_SIA', integer64="warn.loss", verbose = FALSE)
...
if(file.exists(file=paste0(Rdata_dir,"/",df_all_raw_file))) {
  ...
  AWQMS_Data <- AWQMS_Query(HUC8 = sia_huc8,
                            station_IDs =c(),
                            area.Shp = sia_shp,
                            inParms = input$parms,
                            luParms = lu_parms,
...
if(file.exists(file=paste0(Rdata_dir,"/",df_all_clean_file))) {
  ...
  # clip to SIA
  df.all <-df.all[df.all$Station_ID %in% Stations_in_poly(df.all, sia_shp, outside=FALSE),]
  
  
  # Convert AWQMS analyte names to existing code language
  if(any(df.all$Analyte == "Dissolved oxygen")){
    df.all[grep('Dissolved oxygen', df.all$Analyte),]$Analyte <- 'Dissolved Oxygen'
  }
  if(any(df.all$Analyte == "Phosphorus")){
  df.all[grep('Phosphorus', df.all$Analyte),]$Analyte <- 'Total Phosphorus'
  }
  if(any(df.all$Analyte == "Total suspended solids")){
  df.all[grep('Total suspended solids', df.all$Analyte),]$Analyte <- 'Total Suspended Solids'
  }
  if(any(df.all$Analyte == "Temperature, water")){
  df.all[grep('Temperature, water', df.all$Analyte),]$Analyte <- 'Temperature'
  }

  # Remove all Dissolved Oxygen summary statistics from analysis except for 'minimum' 
  df.all <- df.all %>% dplyr::filter(!(Analyte == "Dissolved Oxygen" & Statistical_Base %in% c('7DADM', 'Maximum', 'Mean', '7DADMean', '7DADMin', '30DADMean')))
  ...
  df.temp.ent2 <- unique(df.temp.ent2[(df.temp.ent2$AgArea == sia &  grepl("Coastal Contact Recreation",
  ...
  if (any('Temperature' %in% df.all$Analyte)) {
    df.all <- df.all %>% dplyr::filter(!(Analyte == "Temperature" & Statistical_Base %in% c('7DADM', 'Minimum', 'Mean', '7DADMean', '7DADMin', '30DADMean')))
    tempStns <- temp_sufficiency_analysis(df.all)
    
    if(!is.null(tempStns)){
      qc.1.pass <- dplyr::filter(attributes(tempStns)$day_test, result == "pass")
      qc.2.pass <- dplyr::filter(attributes(tempStns)$month_test, result == "pass")
      qc.3.pass <- dplyr::filter(attributes(tempStns)$year_test, result == "pass")
      qc.1.stn <- dplyr::filter(attributes(tempStns)$day_test, result == "pass")$Station_ID
      qc.2.stn <- dplyr::filter(attributes(tempStns)$month_test, result == "pass")$Station_ID
      qc.3.stn <- dplyr::filter(attributes(tempStns)$year_test, result == "pass")$Station_ID
      ...
      df.all <- df.all %>% dplyr::filter(!Analyte == "Temperature")
    }  
  }
...
```{r Create-other-dataframes, results=FALSE}
df.all$monTest <- "NA"
if (any(c('pH', 'E. Coli', "Enterococcus", "Dissolved Oxygen") %in% df.all$Analyte)) {
  seaken_other <- run_seaKen(dplyr::filter(df.all, Analyte %in% c('pH', 
                                                           'E. Coli', 
                                                           "Enterococcus", 
                                                           "Dissolved Oxygen"
                                                           )))
...
if(NROW(dplyr::filter(df.all, Analyte == "Temperature" & monTest == "pass")) > 0) {
  seaken_temp <- run_seaKen(dplyr::filter(df.all, Analyte == "Temperature" & monTest == "pass"))} else {seaken_temp <- data.frame()}


if(NROW(dplyr::filter(df.all, Analyte == "Total Suspended Solids")) > 0) {
  
  if (input$select %in% c('Lower North Fork Malheur','Owyhee','Malheur River','Burnt River','Powder-Brownlee','Hells Canyon','Lower Snake Asotin')) {
...
if(NROW(dplyr::filter(df.all, Analyte == 'Total Phosphorus')) > 0) {
  
  if (input$select %in% c('Lower North Fork Malheur','Owyhee','Malheur River','Burnt River','Powder-Brownlee','Hells Canyon','Lower Snake Asotin')) {
    # Snake River - Hells Canyon TMDL
    # TMDL target applies during the growing season May - September
    
    seaken_tp <- df.all %>%
      filter(Analyte == 'Total Phosphorus' & month(df.all$Sampled) >= 5 & month(df.all$Sampled) <=9) %>%
      as.data.frame() %>%
      run_seaKen()
    
  } else {
    # Other Area
    seaken_tp <- run_seaKen(filter(df.all, Analyte %in% c('Total Phosphorus')))
  } 
} else {
  seaken_tp <- data.frame()
}

SeaKen <- rbind(seaken_other, seaken_temp, seaken_tss, seaken_tp)
...
#no stns meeting criteria, can't create object
...
#EC-created df.stns by copying df.all instead of subsetting df.all, since no observations meet subset criteria. 
#df.stns <- df.all[df.all$Station_ID %in% unique(stns$Station_ID),]
#df.stns <- df.all[unique(df.all$Station_ID),]
...
#EC- no stns meeting criteria, so no stns object so save. 
...
deq<- paste('Don Butcher','butcher.don@deq.state.or.us', sep = '; ')

oda<- paste('Tom Demianew', 'tdemianew@oda.state.or.us', sep = '; ')

BC_table<-data.frame('Lower North Fork Malheur', deq, oda)
colnames(BC_table) <- c('Strategic Implementation Area', 'DEQ Basin Coordinator', 'ODA Agricultural Water Quality Specialist')
...
Within the `r input$select` Strategic Implementation Area, data from 0 monitoring stations were sufficient to assess status and/or trends out of `r length(unique(df.all$Station_ID))` total monitoring stations. 
...
station_summary_tbl <- data.frame(Analyte=c("_E. coli_","_Enterococcus_","Dissolved Oxygen","pH","Temperature","Total Phosphorus","TSS"),
                                  ...
                                  trends=c(nrow(unique(trend[trend$Analyte == "E. Coli" & trend$t_check, ])),
                                           nrow(unique(trend[trend$Analyte == "Enterococcus"& trend$t_check, ])),
                                           nrow(unique(trend[trend$Analyte == "Dissolved Oxygen"& trend$t_check, ])),
                                           nrow(unique(trend[trend$Analyte == "pH"& trend$t_check,])),
                                           nrow(unique(trend[trend$Analyte == "Temperature"& trend$t_check, ])),
                                           nrow(unique(trend[trend$Analyte == "Total Phosphorus"& trend$t_check, ])),
                                           nrow(unique(trend[trend$Analyte == "Total Suspended Solids"& trend$t_check, ]))
                                           ))
...
knitr::kable(station_summary_tbl, digits = 0, padding = 2, row.names = FALSE, caption = tbls(name = "StnSumTable", caption = "Summary of all monitoring stations within the Strategic Implementation Area"))                                 
...
```{r stations-map-word, eval = word_output, results = "asis", fig.cap=figs(name="stationsmap", caption=paste0("Monitoring station locations within the ", sia, " Agricultural Water Quality Management Area."))}
...
```{r landuse-map-word, eval = word_output, results = "asis", fig.cap=figs(name="landusemap", caption=paste0("Land use and land cover within the ", sia, " Strategic Implementation Area."))}
...
```{r landuse-map-html, results= 'asis', eval = !(word_output), fig.width=8.5, fig.height=7, fig.cap=figs(name="landusemap", caption=paste0("Land use and land cover within the ", sia, " Strategic Implementation Area."))}

sia_shp <-spTransform(sia_shp, CRS("+proj=longlat +datum=NAD83"))

map <- leaflet(sia_shp) %>%
  addPolygons(fill=FALSE, group = "SIA") %>%
  addTiles() %>%
...
# hideGroup('All Stations Queried') %>%
  ...
  addWMSTiles(baseUrl = 'https://edcintl.cr.usgs.gov/geoserver/mrlc_display/wms?',
              group = "Land Use (NLCD 2011)",
              layers = "mrlc_nlcd_2011_landcover_2011_edition_2014_10_10",
              options = WMSTileOptions(version = '1.1.1',
                                       format = 'image/png',
                                       transparent = TRUE)) %>% 
  ...
  addLayersControl(baseGroups = c('Land Use (NLCD 2011)', 'USGS Topo', 'ESRI NatGEO World Map'),
                   overlayGroups = c(
                     # 'Stations That Fit Criteria', 
                     'All Stations Queried', 'Hydrography'),
                   options = layersControlOptions(collapsed = FALSE)) %>%
  addEasyButton(easyButton(
    icon = "fa-globe",
    onClick = JS("function(btn, map){
                var groupLayer = map.layerManager.getLayerGroup('SIA');
...
A summary table of watershed land use by station is presented in `r tbls("landUse", display="cite")`. The table is sorted in descending order by percent agriculture (% Ag), so stations located in watersheds with the highest percent agricultural land uses are at the top of the table.


```{r landuse-table, results="asis"}

#EC-edited to include all stations
stn_to_use<-c(unique(df.all$Station_ID))
...
knitr::kable(landuse, digits = 0, 
             padding = 2, row.names = FALSE, caption = tbls(name = "landUse", 
                                                            caption = "Summary table of watershed land use for all stations in the Strategic Implementation Area."))
...
sia_shp_lambert <- spTransform(sia_shp, CRS("+proj=lcc +lat_1=20 +lat_2=60 +lat_0=40 +lon_0=-96 +x_0=0 +y_0=0 +datum=NAD83 +units=m +no_defs"))
sia_shp_buff <- rgeos::gBuffer(sia_shp_lambert, width=500)
sia_shp_buff <- spTransform(sia_shp_buff, CRS("+init=epsg:4326"))
# sia_shp <- spTransform(sia_shp, CRS("+init=epsg:4326"))
wql_streams_shp <- spTransform(wql_streams_shp, proj4string(sia_shp_buff))
wql_streams_clip <- raster::intersect(wql_streams_shp, sia_shp_buff)

# wq_lim_whole <- extract_303d(df.all, wq_limited, input$select)
wq_lim_whole <- wql_streams_clip@data
...
```{r Seasonal-Kendall, eval = FALSE, results="asis"}

SK <- SeaKen %>% filter(Station_ID %in% unique(status$Station_ID)) %>% 
...
No monitoring stations have data to assess status and/or trend for _E.coli_.

```{r Ecoli, eval = FALSE, fig.height=6, fig.width=7, results='asis', eval.after="fig.cap", fig.cap=mapply(figs, cap.names, ecoli_captions)}
...
if(length(entero_stns) > 0) {
  ...
  cat(paste0("A status and trend assessment was not made for *Enterococcus* because the marine water contact recreation use criteria do not apply in the ",sia,".")) 
} 
...
No monitoring stations have data to assess status and/or trend for Total Phosphorus.

```{r Total-Phosphorus, eval=FALSE, fig.height=6, fig.width=7, results='asis', fig.cap=mapply(figs, cap.names, tp_captions), eval.after="fig.cap"}
...
No monitoring stations have data to assess status and/or trend for Total Suspended Solids.

```{r TSS, eval=FALSE, results = 'asis', fig.width = 7, fig.height = 6, fig.cap=mapply(figs, cap.names, tss_captions), eval.after="fig.cap"}
...
No monitoring stations have data to assess status and/or trend for pH.

```{r pH, eval=FALSE, fig.height=6, fig.width=7, results='asis', fig.cap=mapply(figs, cap.names, pH_captions), eval.after="fig.cap"}
...
No monitoring stations have data to assess status and/or trend for Temperature.

```{r Temperature, eval=FALSE, fig.height=6, fig.width=7, results='asis', fig.cap=mapply(figs, cap.names, temp_captions), eval.after="fig.cap"}
...
No monitoring stations have data to assess status and/or trend for Dissolved Oxygen.

```{r Dissolved-Oxygen, eval= FALSE, fig.height=6, fig.width=7, results='asis', fig.cap=mapply(figs, cap.names, DO_captions), eval.after="fig.cap"}
...
```{r parm-summary-table, eval=FALSE, results = 'asis', eval=FALSE}
...
```{r parm-summary-map-html, eval=FALSE, results = 'asis', eval = !(word_output), fig.width=8.5, fig.height=7}
...
```{r conclusions-setup, eval=FALSE}
...
knitr::kable(org_summary_station, padding = 2, digits = 1, row.names = FALSE, caption = tbls(name = "AppendixorgSumbyStation", caption = "Summary of all unique organizations that collected data at each station for each analyte."))
