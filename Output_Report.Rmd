---
title: '<img src="Figures/LogoColorRegular.jpg" style="float: right;width: 80px; "/>Owyhee Agricultural Water Quality Management Area Water Quality Status and Trends Report'
...
project_dir <- "//deqhq1/WQNPS/Agriculture/Status_and_Trend_Analysis/Owyhee/2018-Owyhee"
...
if(NROW(dplyr::filter(df.all, Analyte == "Total Suspended Solids")) > 0) {
  ...
  seaken_tss_srhc <- data.frame()
}

SeaKen <- rbind(seaken_other, seaken_tss_srhc, seaken_temp)
...
*	Are the Snake River - Hells Canyon sediment and nutrients TMDL targets for total phosphorus and total suspended solids being acheived?
...
The status of water quality standards attainment for dissolved oxygen, *E. coli*, *Enterococcus*, pH, and temperature samples were made in relation to the applicable numeric water quality criterion. Status for total suspended solids and total phosphorus was based on a comparison to the instream TSS and total phosphorus concentration targets established in the Snake River - Hells Canyon Basin sediment, nutrients, nuisance alagae, and dissolved oxygen TMDLs.

Trends were calculated for dissolved oxygen, bacteria, pH, temperature, total phosphorus, and total suspended solids using a Seasonal Kendall test (Hirsch et al. 1982, Hirsch and Slack 1984, and Helsel and Hirsch 2002). A trend result was made at a site if there were a minimum of eight years of available data. A Seasonal Kendall test removes the influence of seasonal fluctuations by calculating a Mann Kendall test (Mann 1945) on each season separately and then comparing the slopes. A significant positive, negative, or steady trend was determined across all seasons and years when the significance of the seasonal slopes had a two-tailed p <= 0.20. A steady trend had a slope equal to zero.   Prior to applying the Seasonal Kendall test data were grouped into monthly "seasons." Multiple observations within any given month were collapsed into a single value using the median. For Total Suspended Solids the monthly mean was calculated instead and observations taken outside the May througth September period were removed. This was done so trend results represent only the period when the Snake River - Hells Canyon sediment TMDL targets apply.
...
Results for total suspended solids were compared against the 50 mg/L monthly average instream TSS concentration loading capacity targets established in the [Snake River - Hells Canyon Sediment TMDL][2.5].

From May through September, results for total phosphorus were compared against the 0.07 mg/L instream total phosphorus concentration loading capacity targets established in the [Snake River - Hells Canyon Sediment TMDL][2.5]. For other months an assessment was not made against a target.
...
[2.5]: https://www.oregon.gov/deq/wq/tmdls/Pages/snakesubbasin.aspx
...
station_summary_tbl <- data.frame(Analyte=c("_E. coli_","_Enterococcus_","Dissolved Oxygen","pH","Temperature","Total Phosphorus","Total Suspended Solids"),
...
if(length(entero_stns) > 0) {
  ...
  print(paste0("A status and trend assessment was not made for the _Enterococcus_ indicator because the marine water contact recreation use criteria do not apply in the Owyhee."))
} 
...
# Set TP instream target (0.07 mg/L May-Sept) from Snake River HC TMDL.
tp_target = list("10729"=0.07,
                 "1119USBR_WQX-OWY101"=0.07,
                 "1119USBR_WQX-OWY110"=0.07)
...
if(length(tp_stn) > 0) {
  ...
  TP <- df.all[df.all$Analyte == 'Total Phosphorus',]
    ...
    for(j in 1: length(tp_stn)) {
      ...
      if(tp_stn[j] %in% names(tp_target)) { 
        TP_evaluate <- EvaluateTPWQS(tmp_df, selectWQSTP = tp_target[[tp_stn[j]]])
        } else {
          # per instructions from John to have all plots with the target
          TP_evaluate <- EvaluateTPWQS(tmp_df, selectWQSTP = 0.07)
        }
...
# Set TSS instream targets (50 mg/L monthly mean) from Snake River HC TMDL. THese are only on stations near the mouth of the Oywhee River.
tss_target = list("10729"=50,
                  "1119USBR_WQX-OWY101"=50,
                  "1119USBR_WQX-OWY110"=50)
...
if(length(tss_stn) > 0){
  ...
  for(j in 1: length(tss_stn)) {
    ...
    if(tss_stn[j] %in% names(tss_target)) {
      TSS_evaluate <- EvaluateTSS_SNRHC(tmp_df, 
                                     TSS_target= tss_target[[tss_stn[j]]])
    } else {
      # per instructions from John D. to include target on all graphs
      TSS_evaluate <- EvaluateTSS_SNRHC(tmp_df, 
                                     TSS_target = 50)
    }
  ...
  colnames(exc) <- c("Station ID","Station Description", "Min Year", "Max Year", 
                     "Obs", "Monthly Exceedances", "% Exceedance")
...
```{r parm-summary-table, include = FALSE, results = 'asis'}
