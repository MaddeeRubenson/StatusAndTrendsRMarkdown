...
if(file.exists(file=paste0(Rdata_dir,"/",df_all_clean_file))) {
  ...
  df.all <- filter(df.all, Station_ID != "USGS-14105700")       #removes a station on the Columbia River outside the area
  df.all <- df.all[!(df.all$Analyte=="pH" & df.all$Result==0),] #removes observations where pH = 0 
} else {
...
if(length(entero_stns) > 0) {
  ...
  print("No monitoring stations have data to assess status and/or trend of _Enterococcus_.") 
} 
