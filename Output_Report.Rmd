---
title: '<img src="Figures/LogoColorRegular.jpg" style="float: right;width: 80px; "/>Umatilla Basin Agricultural Water Quality Management Area Water Quality Status and Trends Report'
...
agwqma <- "Umatilla Basin"
query_dates <- c("2000-01-01", "2018-08-15")
...
if(file.exists(file=paste0(Rdata_dir,"/",df_all_raw_file))) {
  ...
  df.all <- query(select = input$select,
                  HUClist = HUClist,
                  parms = input$parms,
                  dates = c("2000-01-01", "2018-08-15"),
                  db = input$db,
                  lu_parms=lu_parms,
                  stations_huc=stations_huc)
...
if(file.exists(file=paste0(Rdata_dir,"/",df_all_clean_file))) {
  ...
  # Combine CTUIR_WQX-9 and 10404 since they are the same location
  df.all$SD[df.all$Station_ID =="CTUIR_WQX-9"] <- "Water Quality Portal 10404"
  df.all$Station_ID[df.all$Station_ID =="CTUIR_WQX-9"] <- "10404" 
  df.all$Station_Description[df.all$Station_ID =="10404"] <- "Umatilla River at Yoakum Bridge" 
...
*	Are the Umatilla Basin sediment TMDL targets for total suspended solids being met?
...
Two stations located on the Umatilla River at Yoakum Bridge (Station IDs 10404 and CTUIR_WQX-9) are within 60 meters from each other. Becuase of the close distance the availiable data for these two stations were combined. Station ID 10404: Umatilla River at Yoakum Bridge was used in this report to identify the combined data and results.

## Analysis

The status of water quality standards attainment for dissolved oxygen, *E. coli*, *Enterococcus*, pH, and temperature samples were made in relation to the applicable numeric water quality criterion. Status for total suspended solids was based on a comparison to the instream TSS concentration targets established in the Umatilla Basin Sediment TMDL. Status was not determined for total phosphorus samples because there is no applicable water quality criteria or TMDL targets established for this parameter.
...
Results for pH from both grab and continuous sample data were compared to the applicable water quality criterion as found in [OAR 340-041-0021][2.1] and [OAR 340-041-0315][2.1.1].

Results for bacteria samples were compared to the applicable bacteria criterion as found in [OAR 340-041-0009][2.2]. The bacteria standard for freshwater contact recreation is based on the presence of *E. coli* compared to a single sample maximum and a geometric mean of five or more samples in a 90 day period. A status and trend assessment was not made for the _Enterococcus_ or fecal coliform indicators because the marine water contact recreation and shellfish harvesting use criteria do not apply in the Umatilla Basin.
...
Results for total suspended solids were compared against the instream TSS concentration loading capacity targets established in the [Umatilla Basin Sediment TMDL][2.5]. The loading capacity targets range from 60-110 mg/L depending on location.
...
[2.1.1]: https://secure.sos.state.or.us/oard/viewSingleRule.action;JSESSIONID_OARD=kXxU6Bd8vPKFxF8omzLR9w4fSQQ1Fkv2NNyRYIdwJyJpjN3KNpXj!-609023700?ruleVrsnRsn=68934
...
[2.5]: https://www.oregon.gov/deq/wq/tmdls/Pages/TMDLs-Umatilla-Basin.aspx

# Results

Within the `r input$select` AgWQ Management Area, data from `r length(unique(stns$Station_ID))` monitoring stations were sufficient to assess status and/or trends out of `r length(unique(df.all$Station_ID))` total monitoring stations. 
...
```{r landuse-map-html, results= 'asis', eval = !(word_output), fig.width=8, fig.height=8, fig.cap=figs(name="landusemap", caption="Land use and land cover within the Umpqua River AgWQ Management Area.")}
...
knitr::kable(landuse, digits = 0, 
             padding = 2, row.names = FALSE, caption = tbls(name = "landUse", caption = "Summary table of watershed land use for stations with a status or trend result."))
...
if(nrow(SK) > 0) {
  ...
  knitr::kable(SK, padding = 2, format.args = list(scientific = TRUE), row.names = FALSE, caption = tbls(name = "seakenTable", caption = "Seasonal Kendall trend results for each analyte at each monitoring station with at least eight years of data."))
}
...
if(length(entero_stns) > 0) {
  ...
  print("A status and trend assessment was not made for the _Enterococcus_ indicator because the marine water contact recreation use criteria do not apply in the Umatilla Basin.") 
} 
...
```{r Total-Phosphorus, results = 'asis', fig.width = 7, fig.height = 6}
...
if(length(tp_stn) > 0) {
  ...
  TP <- df.all[df.all$Analyte == 'Total Phosphorus',]
    ...
    for(j in 1: length(tp_stn)) {
      ...
      TP_evaluate <- EvaluateTPWQS(tmp_df, selectWQSTP = 0)
...
for (i in 1:length(tp_stn)) {
  ...
  trend_logic<-ifelse(grepl("Not Significant", results_seaken[results_seaken$Station_ID == tp_stn[i],'signif']), FALSE,
                      ifelse(grepl("Need at least 8 years", 
                                   results_seaken[results_seaken$Station_ID == tp_stn[i],'signif']), FALSE, TRUE))
  ...
  print(tp_plots[[i]])
  cat(figs(name = paste0('TP_', tp_stn[i]), 
           caption = paste0('Station ', tp_stn[i], ' Total Phosphorus water quality trends.')))
} 

colnames(exc) <- c("Station ID","Station Description", "Min Date", "Max Date", "Obs", "Exceedances", "% Exceedance")
exc <- exc[,c(1:5)]
knitr::kable(exc,padding = 2, digits = 1, row.names = FALSE, 
             caption = tbls(name = "tpExcTable", caption = "Total Phosphorus observations."))
...
# Set TSS instream targets (mg/L) from Umatilla TMDL.
tss_target = list("10404"=80,
                  "10406"=80,
                  "11489"=76,
                  "12005"=72,
                  "CTUIR_WQX-9"=80)
...
if(!(trend == "No Stations Meet Trend Criteria")) {
  ...
  for(j in 1: length(tss_stn)) {
    ...
    if(tss_stn[j] %in% names(tss_target)) {
      TSS_evaluate <- EvaluateTSSWQS(tmp_df, 
                                     selectWQSTSS = tss_target[[tss_stn[j]]])
    } else {
      TSS_evaluate <- EvaluateTSSWQS(tmp_df, 
                                     selectWQSTSS = 0)
    }
  ...
  for (i in 1:length(tss_stn)) {
    ...
    trend_logic<-ifelse(grepl("Not Significant", results_seaken[results_seaken$Station_ID == tss_stn[i],'signif']), FALSE,
                        ifelse(grepl("Need at least 8 years", 
                                     results_seaken[results_seaken$Station_ID == tss_stn[i],'signif']), FALSE, TRUE))
    ...
    print(tss_plots[[i]])
    cat(figs(name = paste0('TSS_', tss_stn[i]), caption = paste0('Station ', tss_stn[i], ' Total Suspended Solids water quality status and trends.')))
  } 
colnames(exc) <- c("Station ID","Station Description", "Min Date", "Max Date", "Obs", "Exceedances", "% Exceedance")
# exc <- exc[,c(1:5)]
  knitr::kable(exc, padding = 2, digits = 1, row.names = FALSE, caption = tbls(name = "tssExcTable", caption = "Total Suspended Solids observations."))
...
```{r pH, results = 'asis', fig.width = 7, fig.height = 6}
...
if(length(pH_stn_table) > 0) {
  ...
  if(length(pH_stn) > 0) {
    ...
    for (i in 1:length(pH_stn)) {
      ...
      trend_logic<-ifelse(grepl("Not Significant", results_seaken[results_seaken$Station_ID == pH_stn[i],'signif']), FALSE,
                          ifelse(grepl("Need at least 8 years", 
                                       results_seaken[results_seaken$Station_ID == pH_stn[i],'signif']), FALSE, TRUE))
      ...
      cat(figs(name = paste0('pH_', pH_stn[i]), 
               caption = paste0('Station ', pH_stn[i], ' pH water quality status and/or trends.')))
  ...
  if(!is.null(ph_exc)) {
    knitr::kable(ph_exc, padding = 2, digits = 1, row.names = FALSE, 
  ...
  if(length(temp_stn) > 0) {
    ...
    if(length(in_bentable) > 0) {
      ...
      if(length(temp_stn) > 0 & length(sdadm) > 1) {
        ...
        for (i in 1:length(temp_stn)) {
          ...
          if(nrow(new_data) > 1) {
            ...
            print(temp_plots[[i]])
            cat(figs(name = paste0('temp_', temp_stn[i]), 
                     caption = paste0('Station ', temp_stn[i], ' Temperature water quality status and/or trends.')))
      ...
      if(!is.null(exc)){ #prints exceedance table, if temperature stns exist 
        #fix column names
        knitr::kable(exc, padding = 2, digits = 1, row.names = FALSE, caption = tbls(name = "tempExcTable", caption = "Temperature status."))
      }
      
    }
  } else {
    temp <- NULL
    print("There was insufficient data to calculate the seven day average daily maximum stream temperatures and make status and/or trend assessments for stream temperature.")
  }
} else {
  temp <- NULL
...
```{r Dissolved-Oxygen, results = 'asis', fig.width = 7, fig.height = 6}
...
if(any(trend != "No Stations Meet Trend Criteria")) {
  ...
  if(length(DO_stn) > 0) {
    ...
    if(!is.null(exc)){
      knitr::kable(exc, padding = 2, digits = 1, row.names = FALSE, caption = tbls(name = "doExcTable", caption = "Dissolved Oxygen status."))
    }
...
```{r Summary-map1, results = 'asis', eval = word_output & !(split_param_summary), fig.cap=figs(name="Summary_map1", caption="Summary of stations that fit the criteria for status and trend analysis. One or more exceedances within the two whole years of available data defined whether a station was Meeting or Not Meeting. Trend was determined by significant trends associated with long-term datasets.")}
...
save(stns_param_summary_tbl, file=paste0(Rdata_dir,"/", stns_param_summary_file))
...
```{r parm-summary-map-html, results = 'asis', eval = !(word_output), fig.width=8, fig.height=8}
...
knitr::kable(status, padding = 2, digits = 1, row.names = FALSE, caption = tbls(name = "AppendixstatusTable", caption = "Monitoring stations that fit the criteria to assess status; note value represents the number of results for each monitoring station per year."))

knitr::kable(trend, padding = 1, digits = 1, row.names = FALSE, caption = tbls(name = "AppendixtrendTable", caption = "Monitoring stations that fit the criteria to assess trends; note value represents the number of results for each monitoring station per year."))
