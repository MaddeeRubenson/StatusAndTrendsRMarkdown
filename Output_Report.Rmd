...
if(file.exists(file=paste0(Rdata_dir,"/",df_all_clean_file))) {
  ...
  # Remove Enterococcus data from df.all where the Coastal Contact Recreation criterion doesn't apply     #BTD 8/15/18 Do not include the Entero Section, no entero data
  
  # Create a subset of data only including "Enterococcus"
  # df.temp.ent <- df.all[df.all$Analyte == 'Enterococcus',]                                              # BTD 8/15/18 Commented Out
  
  # Create a vector of stations where the Enterococcus criterion apply
  # df.temp.ent2 <- (Ben_use_LU[!is.na(Ben_use_LU$bacteria_use),])                                        # BTD 8/15/18 Commented Out
  # df.temp.ent2 <- unique(df.temp.ent2[(df.temp.ent2$AgArea == agwqma &  grepl("Coastal Contact Recreation", df.temp.ent2$bacteria_use)), c("Station_ID")])   # BTD 8/15/18                                                                                                                                                                Commented Out
  
  # Create vector of stations inside bacteria_nonfreshwater polygon                 
  # df.temp.ent3 <- unique(c(Stations_in_poly(df.temp.ent, bacteria_nonfreshwater_shp, outside=FALSE), df.temp.ent2))     # BTD 8/15/18 Commented Out
 
  
  # Use logic to remove the stations 
  # df.all <- df.all[!(!(df.all$Station_ID %in% df.temp.ent3) & df.all$Analyte == 'Enterococcus'),]                       # BTD 8/15/18 Commented Out
...
# BTD 8/15/18 Commented out this section due to Evaluation error: object 'monTest' not found 
if(NROW(qc.3.pass) > 0) {
  seaken_temp <- run_seaKen(dplyr::filter(df.all, Analyte == "Temperature" & monTest == "pass"))
  } else {seaken_temp <- data.frame()}

# BTD 8/15/18 Added line below in conj. with commenting out above section
#seaken_temp <- data.frame()                     
...
writeOGR(obj=all_stn_sp, dsn=GIS_dir, layer="stations_all", driver="ESRI Shapefile", overwrite_layer=TRUE)
...
knitr::kable(landuse, digits = 0, row.names = FALSE,  
...
exc <- exc[,c(1:5)]
knitr::kable(exc,padding = 2, digits = 1, row.names = FALSE, caption = tbls(name = "tpExcTable", caption = "Total Phosphorus observations."))
...
if(any(trend$Analyte == 'Temperature') | any(status$Analyte == 'Temperature')) {
  ...
  print("There are no stations with sufficient available data to assess status or trends for temperature")
  temp <- NULL
}
temp_exc <- exc
