...
if(file.exists(file=paste0(Rdata_dir,"/",df_all_clean_file))) {
  ...
  if (any('Temperature' %in% df.all$Analyte)) {
    df.all <- df.all %>% filter(!(Analyte == "Temperature"))
...
trend <- Stations_Trend(df.all=df.all, SeaKen=SeaKen)
...
The status of water quality standards attainment for dissolved oxygen, *E. coli*, *Enterococcus*, pH, and temperature samples were made in relation to the applicable numeric water quality criterion. For some waterbodies the applicable water quality criterion for these parameters is a narrative non-numeric criterion. In this situation, status was not determined. Status was also not determined for total suspended solids samples because there is no applicable water quality criteria or concentration based TMDL target. Status for total phosphorus data collected in Tenmile Lake were based on a comparison to the the 0.0071 mg/L total phosphorus concentration target established in the [Tenmile Lakes Watershed TMDL][2.2.5]. A status assessment was made at a monitoring station if data were available within the last whole two years in the period from `r format(floor_date(ymd(input$dates[2]) - years(2), unit="year"), format="%B %d, %Y")` to `r format(ymd(input$dates[2]), format="%B %d, %Y")`. 
...
Total phosphorus results for stations in Tenmile Lake were compared against the lake water column 0.0071 mg/L total phosphorus concentration target established in the [Tenmile Lakes Watershed TMDL][2.2.5]. The target is expressed as an annual mean so data were also summarized as annual means. 
...
[2.2.5]: https://www.oregon.gov/deq/FilterDocs/scTenmiletmdl.pdf
...
The map below shows the general location of monitoring stations that fit the status and/or trend data requirements along with the distribution of federal and non-federal lands. 
...
if(length(tp_stn) > 0) {
  ...
  for(j in 1: length(tp_stn)) {
    ...
    if (tp_stn[j] %in% c('14018','34828','34829','34830','CTCLUSI_WQX-CTCTDWQ')) {
      # Tenmile Lakes TMDL
      TP_evaluate <- EvaluateTP_Tenmile(new_data,
                                        selectWQSTP = 0.0071) # mg/L
    } else {
...
# format for table
stns_param_summary_tbl <- dplyr::select(stns_param_summary, -c(DECIMAL_LAT, DECIMAL_LONG, TSS_S))
...
conclusions.TP <- "Data were not available in sufficient quantity to assess status within the last two whole years, including stations located on Tenmile Lakes.  Data collected at nine stations were sufficient to assess trends. Four of the nine stations (10393, 10596, 13570, 13574) had a steady trend. The remaining five stations (11485, 11486, 14018, 34829, 34830) did not have statistically significant trends."

conclusions.TSS <- "Data collected at six stations were sufficient to assess trends. Three of the six stations (10393, 11485, 11486) had a degrading trend. The remaining three stations (10596, 13570, 13574) did not have statistically significant trends."
