---
title: '<img src="Figures/LogoColorRegular.jpg" style="float: right;width: 80px; "/>Curry County Agricultural Water Quality Management Area Water Quality Status and Trends Report'
...
project_dir <- "N:/Agriculture/Status_and_Trend_Analysis/Curry County/2018-Curry-County"
...
support_files_dir <- "N:/Agriculture/Status_and_Trend_Analysis/R_support_files"
...
if (!word_output) {
  ...
  figs(name="fig1_result_map", display=FALSE)
  figs(name="fig2_result_map", display=FALSE)
...
wq_limited <- read.csv('Lookups/wq_limited_2012_Curry_County.txt')
...
[1.1]: https://secure.sos.state.or.us/oard/displayDivisionRules.action?selectedDivision=2759
[1.2]: https://www.oregon.gov/ODA/shared/Documents/Publications/NaturalResources/CurryCountyAWQMAreaPlan.pdf
[1.3]: https://www.oregonlegislature.gov/bills_laws/ors/ors568.html
...
Dissolved oxygen status was assessed by comparing the measured concentration values to the applicable daily minimum water quality criterion found in [OAR 340-041-0016][2.3]. If the dissolved oxygen concentration exceeded the water quality criterion, but met the criteria for percent saturation at the same time, it was considered to meet the water quality criterion. These points were noted in the plots using a different shape. Assessments were not made against the seven and 30-day mean minimum dissolved oxygen criteria due to the general lack of continuous dissolved oxygen data.
...
```{r landuse-map-html, results= 'asis', eval = !(word_output), fig.width=10, fig.height=8, fig.cap=figs(name="landusemap", caption="Land use and land cover within the Curry County River AgWQ Management Area.")}
...
```{r Total-Phosphorus, results = 'asis', fig.height=6, fig.width=7, }
...
if(any(!(trend == "No Stations Meet Trend Criteria"))) {
  ...
  if(!is.null(ph_exc)) {
    knitr::kable(ph_exc, padding = 2, digits = 1, row.names = FALSE, caption = tbls(name = "phExcTable", caption = "pH status."))
  }
  # } else {
...
```{r Temperature, results = 'asis', fig.width = 7, fig.height = 6}
      ...
      if(length(temp_stn) > 0 & length(sdadm) > 1) {
        ...
        for (i in 1:length(temp_stn)) {
          ...
          if(nrow(new_data) > 1) {
            ...
            b <- plot.Temperature(new_data = temp_evaluate[temp_evaluate$Station_ID==temp_stn[i],],
                                  all_data = df.all,
                                  sea_ken_table = SeaKen,
                                  selectUse = Temp_Benuse[1],
                                  selectSpawning = spwn_time[1],
...
agarea_shp_buff <- gBuffer(agwqma_shp[agwqma_shp$PlanName == input$select,], width=1000)
