...
agwqma <- "Generic"
query_dates <- c("2000-01-01", "2018-11-01")
project_dir <-  "//deqhq1/WQNPS/Agriculture/Status_and_Trend_Analysis/Enhancement-Branches/enhancement-awqms-query"
...
source('functions/AWQMS_Query.R')
...
if(file.exists(file=paste0(Rdata_dir,"/",df_all_clean_file))) {
  ...
  df.all <- df.all %>% filter(!(Analyte == "Dissolved oxygen (DO)" & Statistical_Base %in% c('7DADM', 'Maximum', 'Mean', '7DADMean', '7DADMin', '30DADMean')))
...
Conc_LU[(Conc_LU$AgArea %in% input$select & Conc_LU$topic == "DO"), c("conclusion")] <- conclusions.DO
Conc_LU[(Conc_LU$AgArea %in% input$select & Conc_LU$topic == "Ecoli"), c("conclusion")] <- conclusions.Ecoli
Conc_LU[(Conc_LU$AgArea %in% input$select & Conc_LU$topic == "Entero"), c("conclusion")] <- conclusions.Entero
Conc_LU[(Conc_LU$AgArea %in% input$select & Conc_LU$topic == "pH"), c("conclusion")] <- conclusions.pH
Conc_LU[(Conc_LU$AgArea %in% input$select & Conc_LU$topic == "Temp"), c("conclusion")] <- conclusions.Temp
Conc_LU[(Conc_LU$AgArea %in% input$select & Conc_LU$topic == "TP"), c("conclusion")] <- conclusions.TP
Conc_LU[(Conc_LU$AgArea %in% input$select & Conc_LU$topic == "TSS"), c("conclusion")] <- conclusions.TSS
