---
title: '<img src="Figures/LogoColorRegular.jpg" style="float: right;width: 80px; "/>Lost River Agricultural Water Quality Management Area Water Quality Status and Trends Report'
...
agwqma <- "Lost River"
query_dates <- c("2000-01-01", "2018-10-01")
project_dir <-  "//deqhq1/WQNPS/Agriculture/Status_and_Trend_Analysis/Lost River/2018-Lost-River"
...
if(file.exists(file=paste0(Rdata_dir,"/",df_all_clean_file))) {
  ...
  # Combine stations at Klamath straits drain
  df.all$Station_ID[df.all$Station_ID =="USGS-420451121510000"] <- "USGS-420450121504500" 
  df.all[df.all$Station_ID =="USGS-420450121504500", "Station_Description"] <- "KLAMATH STRAITS DRAIN AT HIGHWAY 97, OR"
  df.all[df.all$Station_ID =="USGS-420450121504500", "DECIMAL_LAT"] <- 42.08063
  df.all[df.all$Station_ID =="USGS-420450121504500", "DECIMAL_LONG"] <- -121.8458
  
  # Combine stations at Klamath River at Hwy 66
  df.all$Station_ID[df.all$Station_ID =="USGS-11509370"] <- "10765" 
  df.all[df.all$Station_ID =="10765", "Station_Description"] <- "Klamath River at Hwy 66 (Keno)"
  df.all[df.all$Station_ID =="10765", "DECIMAL_LAT"] <- 42.12761
  df.all[df.all$Station_ID =="10765", "DECIMAL_LONG"] <- -121.9284
  
  # Combine stations at Klamath River at Hwy 66
  df.all$Station_ID[df.all$Station_ID =="CEDEN-KR24600_KHSA"] <- "USGS-420853121505501" 
  df.all[df.all$Station_ID =="USGS-420853121505501","Station_Description"] <- "KLAMATH RIVER AT MILLER ISLAND BOAT RAMP-BOTTOM"
  df.all[df.all$Station_ID =="USGS-420853121505501", "DECIMAL_LAT"] <- 42.14806
  df.all[df.all$Station_ID =="USGS-420853121505501", "DECIMAL_LONG"] <- -121.8486
  
  # Combine stations at Klamath River at Hwy 66
  df.all$Station_ID[df.all$Station_ID =="USGS-11507501"] <- "10768" 
  df.all$Station_ID[df.all$Station_ID =="CEDEN-KR25444_KHSA"] <- "10768"
  df.all[df.all$Station_ID =="10768","Station_Description"] <- "Link River at mouth (Klamath Falls)"
  df.all[df.all$Station_ID =="10768", "DECIMAL_LAT"] <- 42.2185
  df.all[df.all$Station_ID =="10768", "DECIMAL_LONG"] <- -121.7889
...
```{r exec-summary-placeholder, eval=FALSE}
...
```{r landuse-exec, eval=Executive_Summary_check}
...
```{r exec-parm-summary-table, eval=Executive_Summary_check, results='asis'}
...
```{r exec-Entero-summary-map, eval = FALSE, results='asis', fig.cap=figs(name="exec-Entero-summary-map", caption="Summary of stations that fit the criteria for status and trend analysis for _Enterococcus_. One or more exceedances within the last three years of available data defined whether a station was Meeting or Not Meeting. Trend was determined by significant trends associated with long-term datasets.")}
...
```{r exec-auto-conclusion, results='asis', eval=Executive_Summary_check}
conclusions <- list()
for(i in unique(SeaKen$analyte)){
  conclusions[[i]] <- con.auto(stns_param_summary, i)
  cat(paste0("* *", i, "*: ", conclusions[[i]], "\n"))
}
# conclusions
```
...
Four sets of stations were within 60 meters from each other. Because of the close distance the available data for these stations were combined. USGS-420450121504500 includes data from USGS-420451121510000 to represent Klamath Straits Drain near Highway 97. Station 10765 includes data from USGS-11509370, representing similar locations on the Klamath River near Highway 66. Klamath River near Miller Island is represented by USGS-420853121505501 which incorporates data from CEDEN-KR24600_KHSA. Station 10768 combines data from both USGS-11507501 and CEDEN-KR25444_KHSA to represent Link River near the mouth.
...
Results for total phosphorus were compared against the target total phosphorus water quality compliance concentrations used to establish load allocations in the Upper Klamath and Lost River Subbasins Nutrient TMDL. The target concentrations range from 0.024 mg/L to .0.069 mg/L depending on the location.
...
The map in Figure `r figs("stationsmap", display="num")` shows the general location of monitoring stations that fit the status and/or trend data requirements along with the distribution of federal and non-federal lands. 


```{r stations-map-word, eval = word_output, results = "asis", fig.cap=figs(name="stationsmap", caption=paste0("Monitoring station locations within the ", agwqma, " Agricultural Water Quality Management Area.")), fig.height=8}
...
The map in Figure `r figs("landusemap", display="num")` shows station location in relation to the 2011 national land cover dataset (Homer et al. 2015).

```{r landuse-map-word, eval = word_output, results = "asis", fig.cap=figs(name="landusemap", caption=paste0("Land use and land cover within the ", agwqma, " Agricultural Water Quality Management Area.")), fig.height=8}
...
```{r landuse-map-html, results= 'asis', eval = !(word_output), fig.width=8, fig.height=8, fig.cap=figs(name="landusemap", caption=paste0("Land use and land cover within the ", agwqma, " Agricultural Water Quality Management Area."))}
...
if(length(ecoli_stns) > 0) {
  ...
  for(j in 1: length(ecoli_stns)) {
    ...
    print(b)
    # cat(paste0('![Station ', ecoli_stns[j], ' _E. coli_ water quality status and/or trends.]()'))
    # cat(figs(name = paste0('E_', ecoli_stns[j]), caption = paste0('Station ', ecoli_stns[j], ' _E. coli_ water quality status and/or trends.')))
    # paste0('![Station ', ecoli_stns[j], ' _E. coli_ water quality status and/or trends.](', print(b), ')')
  ...
  colnames(exc) <- c("Station ID", "Station Description","Observations", "Exceedances", "Percent Exceedance")
...
if(length(entero_stns) > 0) {
  ...
  for (i in 1:length(entero_stns)) {
    ...
    entero_captions[[i]] <- paste0('Station ', entero_stns[i], ' _Enterococcus_ water quality status and/or trends.')
    cap.names[[i]] <- paste0("entero.",entero_stns[i])
    cat('\n\n')
  }
  ...
  entero_stns <- NULL
  cat(paste0("A status and trend assessment was not made for _Enterococcus_ because the marine water contact recreation use criteria do not apply in the ",agwqma,".")) 
} 
...
if(length(tp_stn) > 0) {
  ...
  for(j in 1: length(tp_stn)) {
    ...
    tp_stn_name <- unique(TP_all[TP_all$Station_ID == tp_stn[j],]$Station_Description)
    ...
    if (tp_stn_name %in% c("Link River at mouth (Klamath Falls)")) {
      # Snake River - Hells Canyon TMDL
      TP_evaluate <- EvaluateTPWQS(tmp_df, 
                                     selectWQSTP = 0.024)
    } else if (tp_stn_name %in% c("KLAMATH STRAITS DRAIN AT TOWNSHIP RD", 
                                  "KLAMATH STRAITS DRAIN AT HIGHWAY 97, OR", 
                                  "ADY CANAL AT HIGHWAY 97, NEAR WORDEN, OR",
                                  "Klamath Strait at USBR Pump Station F")){
      TP_evaluate <- EvaluateTPWQS(tmp_df, 
                                     selectWQSTP = 0.035)
    } else {
...
if(length(tss_stn) > 0){
  ...
  for(j in 1: length(tss_stn)) {
    new_data <- TSS_all[TSS_all$Station_ID == tss_stn[j],]
...
if(any(!(trend == "No Stations Meet Trend Criteria"))) {
  ...
  if(length(pH_stn) > 0) {
    ...
    for (i in 1:length(pH_stn)) {
      ...
      pH_captions[[i]] <- paste0('Station ', pH_stn[i], ' pH water quality status and/or trends.')
      cap.names[[i]] <- paste0("pH.",pH_stn[i])
      cat('\n\n')
    } 
  ...
  colnames(ph_exc) <- c("Station ID",	"Station Description",	"Min Date",	"Max Date",	"Observations", "Exceedances", "Percent Exceedance")
...
if(length(temp_stn) > 0) {
  ...
  #temp_all <- df.all[df.all$Analyte == 'Temperature',]
  ...
  for(j in 1: length(temp_stn)) {
    ...
    new_data <- temp[temp$Station_ID == temp_stn[j],]
  ...
  knitr::kable(exc, padding = 2, digits = 1, row.names = FALSE, 
    ...
    temp <- NULL
    print(paste0("There was insufficient data to calculate the seven day average daily maximum stream temperatures and make status and/or trend assessments for stream temperature."))
...
if(length(DO_stn) > 0) {
  ...
  if(any(!is.na(in_bentable))) {
    ...
    for (i in 1:length(DO_stn)) {
      ...
      DO_captions[[i]] <- paste0('Station ', DO_stn[i], ' Dissolved Oxygen water quality status and/or trends.')
      cap.names[[i]] <- paste0("DO.",DO_stn[i])
      cat('\n\n')
    }
  }
  ...
  colnames(exc) <- c("Station ID", "Station Description", "Observations", "Exceedances", "Meets b/c %Sat", "Min Year", "Max Year")
  if(!is.null(exc)){
...
```{r Entero-summary-map, eval = FALSE, results='asis', fig.cap=figs(name="Entero_map", caption="Summary of stations that fit the criteria for status and trend analysis for Enterococcus. One or more exceedances within the last three years of available data defined whether a station was Meeting or Not Meeting. Trend was determined by significant trends associated with long-term datasets.")}
...
knitr::kable(stns_param_summary_tbl[,c(1:6,9:10)],
             padding = 2, digits = 3,
             row.names = FALSE,
             caption = tbls(name = "parmSumTable1", 
                            caption = "Summary of monitoring stations status and trend, where 'Exceeds' represents a single exceedance of the water quality standard or TMDL target within the last two whole years. Note: TSS = total suspended solids."))
cat("\n\n")
knitr::kable(stns_param_summary_tbl[,c(1,2,7,8,11:16)],
             padding = 2, digits = 3,
             row.names = FALSE,
             caption = tbls(name = "parmSumTable2", 
                            caption = paste0("Continued summary of monitoring stations status and trend, where 'Exceeds' represents a single exceedance of the water quality standard or TMDL target within the last two whole years. See ", tbls("parmSumTable1", cite = ), " Note: DO = dissolved oxygen, TP = total phosphorus.")))
...
```{r auto-conclusion, results='asis'}
conclusions <- list()
for(i in unique(SeaKen$analyte)){
  conclusions[[i]] <- con.auto(stns_param_summary, i)
  cat(paste0("* *", i, "*: ", conclusions[[i]], "\n"))
...
<!-- * *Dissolved Oxygen*: `r conclusions[['Dissolved Oxygen']]` -->
<!-- * *E. coli*: `r conclusions[['E.Coli']]` -->
<!-- * *Enterococcus*: `r conclusions[['Enterococcus']]` -->
<!-- * *pH*: `r conclusions[['pH']]` -->
<!-- * *Temperature*: `r conclusions[['Temperature']]` -->
<!-- * *Total Phosphorus*: `r conclusions[['Total Phosphorus']]` -->
<!-- * *Total Suspended Solids*: `r conclusions[['Total Suspendid Solids']]` -->
