...
title: '<img src="LogoColorRegular.jpg" style="float: right;width: 80px; "/> Oregon
  DEQ Water Quality Status and Trends report for the Middle John Day AgWQ
  Management Area'
author: ''
date: "April 2018"
output:
...
input$select <- "Molalla-Pudding French Prairie North Santiam" #input
...
} else if (input$select == "Middle John Day") {
  load(file = "//deqhq1/tmdl/AgWQM/DataAnalysis/Middle John Day/2018/Middle John Day.df.all.2018-03-12.Rdata")
...
# save(df.all, file = paste0("RdataFiles/", input$select, ".df.all.", Sys.Date(), ".Rdata"))
  # save(df.all, file = "//deqhq1/tmdl/AgWQM/DataAnalysis/Molalla-Pudding French Prairie North Santiam/2018/MolallaPuddingFPNS_2018-04-04.Rdata")
...
if (any('Temperature' %in% df.all$Analyte)) {
  ...
  sdadm <- df.all %>% mutate(date = date(Sampled)) %>% filter(Analyte == "Temperature") %>% merge(qc.1.pass[,c("Station_ID", "date", "result")], by=c("Station_ID", "date"), all=FALSE) %>% Calculate.sdadm("Result", "Station_ID", "Sampled", '%Y-%m-%d %H:%M:%S')
...
## Basin Contact
```{r basin contact, echo = FALSE, message = FALSE, warning=FALSE, error = TRUE, results="asis"}
library(knitr)
library(dplyr)
library(plyr)
...
```{r landuse table, echo = FALSE, results='asis'}
...
kable(wq_limited_df, padding = 2, caption = tbls(name = "wqLimited", caption = "Summary of Integrated Report listings for parameters included in this report. Table based on the approved (and partially disapproved) 2012 Integrated Report Listings by the EPA**"))
...
```{r Ecoli, echo = FALSE, results = 'asis', warning=FALSE, error = TRUE, fig.width = 6, fig.height = 6}
...
if(!is.null(exc)) {
  colnames(exc) <- c("Station ID", "Station Description", "Sample", "Obs", "Exceedances", "% Exceedance")
    kable(exc, padding = 2, digits = 1, caption = tbls(name = "bacExcTable", caption = "E.coli status and trends, if sufficient data exists to calculate the geometric mean, it is included in the table.**"))
}
```
...
```{r Entero, eval=FALSE, results = 'asis', error=TRUE, fig.cap='Entero water quality status and trends', fig.height=6, fig.width=6, message=FALSE, warning=FALSE, include=FALSE}
source("functions/funPlots.R")
library(dplyr)
library(ggplot2)
library(knitr)
...
if(any(df.all$Analyte == 'Enterococcus')){
  ...
  if(!(trend == "No Stations Meet Trend Criteria")) {
    ...
    for (i in 1:length(ent_stns)) {
      ...
      ent_plots[[i]] <- b
      cat(figs(name = paste0('Entero_', ent_stns[i]), caption = paste0('Station: ', ent_stns[i], ' _Enterococcus_ water quality status and trends**')))
    }
    
    kable(exc, padding = 2, digits = 1, caption = tbls(name = "enterExcTable", 
...
```{r Total Phosphorus, echo = FALSE, results = 'asis', message = FALSE, warning=FALSE, error = TRUE, fig.width = 6, fig.height = 6, fig.cap = 'Total phosphorus water quality status and trends'}
source("functions/funPlots.R")
...
exc <- exc[,c(1:5)]
kable(exc,padding = 2, digits = 1, caption = tbls(name = "tpExcTable", caption = "Total Phosphorus status and trends**"))
...
```{r TSS, echo = FALSE, results = 'asis', message = FALSE, warning=FALSE, error = TRUE, fig.width = 6, fig.height = 6, fig.cap = 'Total Suspended Solids water quality status and trends'}
source("functions/funPlots.R")
library(dplyr)
library(ggplot2)
library(knitr)
...
colnames(exc) <- c("Station ID","Station Description", "Min Date", "Max Date", "Obs", "Exceedances", "% Exceedance")
exc <- exc[,c(1:5)]
  kable(exc, padding = 2, digits = 1, caption = tbls(name = "tssExcTable", caption = "Total Suspended Solids status and trends**"))
...
```{r pH, echo = FALSE, results = 'asis', message = FALSE, warning=FALSE, error = TRUE, fig.width = 6, fig.height = 6, fig.cap = 'pH water quality status and trends'}
source("functions/funPlots.R")
library(dplyr)
library(ggplot2)
library(knitr)
...
if(any(!(trend == "No Stations Meet Trend Criteria"))) {
  ...
  if(!is.null(ph_exc)) {
    kable(ph_exc, padding = 2, digits = 1, caption = tbls(name = "phExcTable", caption = "pH status and trends**"))
  }
  # } else {
...
```{r Temperature, echo = FALSE, results = 'asis', message = FALSE, warning=FALSE, error = TRUE, fig.width = 6, fig.height = 6, fig.cap = 'Temperature water quality status and trends'}
source("functions/funPlots.R")
library(dplyr)
library(ggplot2)
library(knitr)
  ...
  if(length(temp_stn) > 0) {
    ...
    if(length(in_bentable) > 0) {
      ...
      if(!is.null(exc)){ #prints exceedance table, if temperature stns exist 
       #fix column names
         kable(exc, padding = 2, digits = 1, caption = tbls(name = "tempExcTable", caption = "Temperature status and trends**"))
      }
...
```{r Dissolved Oxygen, echo = FALSE, results = 'asis', message = FALSE, warning=FALSE, error = TRUE, fig.width = 6, fig.height = 6, fig.cap = 'Dissolved Oxygen water quality status and trends'}
source("functions/funPlots.R")
library(dplyr)
library(ggplot2)
library(knitr)
...
if(any(trend != "No Stations Meet Trend Criteria")) {
  ...
  if(length(DO_stn) > 0) {
    ...
    if(!is.null(exc)){
      kable(exc, padding = 2, digits = 1, caption = tbls(name = "doExcTable", caption = "Dissolved Oxygen status and trends**"))
    }
...
kable(stnsformap, padding = 2, digits = 3, caption = tbls(name = "parmSumTable", caption = "Summary of Monitoring Stations Status and Trend, where 'exceed' represents a single exceedance of the water quality standard within the last two years of available data. Note: DO = dissolved oxygen**"))
...
kable(status, padding = 2, digits = 1, caption = tbls(name = "statusTable", caption = "Monitoring stations that fit the criteria to assess status; note value represents the number of results for each monitoring station per year**"))

kable(trend, padding = 1, digits = 1, caption = tbls(name = "trendTable", caption = "Monitoring stations that fit the criteria to assess trends; note value represents the number of results for each monitoring station per year**"))

kable(stns_by_year, padding = 1, digits = 1, caption = tbls(name = "stnByYrTable", caption = "All monitoring stations that were queried and the number of results per year from 2000 to 2017**"))
