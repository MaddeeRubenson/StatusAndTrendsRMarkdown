---
title: '<img src="Figures/LogoColorRegular.jpg" style="float: right;width: 80px; "/>Lower Powder Strategic Implementation Area Water Quality Status and Trends Report'
...
project_dir <-  "//deqhq1/WQNPS/Agriculture/Status_and_Trend_Analysis/SIAs/2019-SIA-Lower-Powder"
...
# import GIS features
# SIA area shp
sia_shp <- readOGR(dsn = GIS_dir, layer = 'LowerPowder_SIA', integer64="warn.loss", verbose = FALSE)
...
if(NROW(dplyr::filter(df.all, Analyte == 'Total Phosphorus')) > 0) {
  ...
  if (input$select %in% c('Owyhee','Malheur River','Burnt River','Powder-Brownlee','Hells Canyon','Lower Snake Asotin')) {
    ...
    seaken_tp <- df.all %>%
      filter(Analyte == 'Total Phosphorus' & month(new_data$Sampled) >= 5 & month(new_data$Sampled) <=9) %>%
      as.data.frame() %>%
      run_seaKen()
...
deq<- paste('John Dadoly','dadoly.john@deq.state.or.us', sep = '; ')

oda<- paste('Tom Demianew', 'tdemianew@oda.state.or.us', sep = '; ')

BC_table<-data.frame('Lower Powder', deq, oda)
