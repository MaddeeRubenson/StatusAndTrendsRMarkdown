...
knitr::kable(station_summary_tbl, digits = 0, padding = 2, caption = tbls(name = "StnSumTable", caption = "Summary of stations with sufficient data for status or trend analysis"))                                 
...
For all monitoring stations with sufficient data (8 years or more), trending was performed using Seasonal Kendall trend analysis. Results are summarized in `r tbls(name = "seakenTable", display="cite")` below. If no table is visible, there were no monitoring stations with sufficient data to assess trends.
...
for (i in 1:length(tp_stn)) {
  ...
  print(tp_plots[[i]])
  cat(figs(name = paste0('TP_', tp_stn[i]), caption = paste0('Station ', tp_stn[i], ' Total Phosphorus water quality trends.')))
} 
...
# }else {
#   print("No monitoring stations have sufficient data to assess status and/or trend of total phosphorus")
# }
#} 
...
if(!(trend == "No Stations Meet Trend Criteria")) {
  ...
  for (i in 1:length(tss_stn)) {
    ...
    print(tss_plots[[i]])
    cat(figs(name = paste0('TSS_', tss_stn[i]), caption = paste0('Station ', tss_stn[i], ' Total Suspended Solids water quality trends.')))
  } 
colnames(exc) <- c("Station ID","Station Description", "Min Date", "Max Date", "Obs", "Exceedances", "% Exceedance")
# exc <- exc[,c(1:5)]
...
# }else {
#   "No monitoring stations have sufficient data to assess trends in total suspended solids concentration."
# }
}
tss_exc <- exc
...
if(any(!(trend == "No Stations Meet Trend Criteria"))) {
  ...
  if(length(pH_stn) > 0) {
    ...
    for (i in 1:length(pH_stn)) {
      ...
      cat(figs(name = paste0('pH_', pH_stn[i]), caption = paste0('Station ', pH_stn[i], ' pH water quality status and/or trends.')))
      ...
      if(length(temp_stn) > 0 & length(sdadm) > 1) {
        ...
        for (i in 1:length(temp_stn)) {
          ...
          if(nrow(new_data) > 1) {
            ...
            print(temp_plots[[i]])
            cat(figs(name = paste0('temp_', temp_stn[i]), caption = paste0('Station ', temp_stn[i], ' Temperature water quality status and/or trends.')))
    ...
    #results_seaken<-SeaKen %>% filter(analyte == 'Dissolved Oxygen')
      ...
      for (i in 1:length(DO_stn)) {
       ...
       if(DO_stn[i] %in% contstns) {
        ...
        print(DO_plots[[i]])
        cat(figs(name = paste0('DO_', DO_stn[i]), caption = paste0('Station ', DO_stn[i], ' Dissolved Oxygen water quality status and/or trends.')))
      }
    }
