---
title: '<img src="Figures/LogoColorRegular.jpg" style="float: right;width: 80px; "/>Klamath Headwaters Agricultural Water Quality Management Area Water Quality Status and Trends Report'
...
output:
  ...
  word_document:
    fig_caption: yes
    reference_docx: N:/Agriculture/Status_and_Trend_Analysis/R_support_files/Report_Template.docx
    toc: yes
...
agwqma <- "Klamath Headwaters"
query_dates <- c("2000-01-01", "2018-12-01")
project_dir <-  "//deqhq1/WQNPS/Agriculture/Status_and_Trend_Analysis/Klamath Headwaters/2018-Klamath-Headwaters"
...
if(file.exists(file=paste0(Rdata_dir,"/",df_all_clean_file))) {
  ...
  # Combine stations at Jack Creek
  df.all[df.all$Station_ID %in% c("FWNF-077", "FWNF-080"),"Station_ID"] <- "FWNF-085"
  df.all[df.all$Station_ID =="FWNF-085", "Station_Description"] <- "JK Group 4A_LT"
  df.all[df.all$Station_ID =="FWNF-085", "DECIMAL_LAT"] <- 43.23447
  df.all[df.all$Station_ID =="FWNF-085", "DECIMAL_LONG"] <- -121.6119
  
  # # Combine stations at Upper Klamath Lake Middle Lake North
  # df.all[df.all$Station_ID %in% c("422622122004003"),"Station_ID"] <- "KL0008"
  # df.all[df.all$Station_ID =="KL0008", "Station_Description"] <- "UPPER KLAMATH LAKE AT MIDDLE LAKE NORTH"
  # df.all[df.all$Station_ID =="KL0008", "DECIMAL_LAT"] <- 42.44136
  # df.all[df.all$Station_ID =="KL0008", "DECIMAL_LONG"] <- -121.9988
  
  # Combine stations on Threemile Creek
  df.all[df.all$Station_ID %in% c("FWNF-162"),"Station_ID"] <- "FWNF-161"
  df.all[df.all$Station_ID =="FWNF-161", "Station_Description"] <- "ThreemileCr_TM4900_LTWT"
  df.all[df.all$Station_ID =="FWNF-161", "DECIMAL_LAT"] <- 42.63556
  df.all[df.all$Station_ID =="FWNF-161", "DECIMAL_LONG"] <- -122.1100
  
  # Combine stations on Threemile Creek
  df.all[df.all$Station_ID %in% c("11504115"),"Station_ID"] <- "WR4000"
  df.all[df.all$Station_ID =="WR4000", "Station_Description"] <- "WOOD RIVER AT DIKE ROAD"
  df.all[df.all$Station_ID =="WR4000", "DECIMAL_LAT"] <- 42.58158
  df.all[df.all$Station_ID =="WR4000", "DECIMAL_LONG"] <- -121.9404
  ...
  df.all$Result <- ifelse(df.all$Unit == 'ug/l', df.all$Result * 0.001 , df.all$Result)
  df.all$Unit <- ifelse(df.all$Unit == 'ug/l', 'mg/l' , df.all$Unit)
...
Results for pH from both grab and continuous sample data were compared to the applicable water quality criterion as found in [OAR 340-041-0021][2.2.2].
...
[2.2.1]: https://www.oregon.gov/deq/FilterDocs/UKtmdlwqmp.pdf
[2.2.2]: https://secure.sos.state.or.us/oard/viewSingleRule.action;JSESSIONID_OARD=Q--AbU9sI2gNpiU9aa_3IO93qbFAoGOHlZ2WHGSe8IsE0OGtWMuS!568786841?ruleVrsnRsn=68713
...
[2.2.5]: https://secure.sos.state.or.us/oard/viewSingleRule.action;JSESSIONID_OARD=Q--AbU9sI2gNpiU9aa_3IO93qbFAoGOHlZ2WHGSe8IsE0OGtWMuS!568786841?ruleVrsnRsn=244176
...
map <- leaflet(agwqma_shp) %>%
  ...
  addLayersControl(baseGroups = c(
    'Land Use (NLCD 2011)', 'USGS Topo',
    'ESRI NatGEO World Map'),
                   overlayGroups = c('Stations That Fit Criteria'
  ...
  for(j in 1: length(tp_stn)) {
    ...
    if (input$select %in% c('Owyhee','Malheur River','Burnt River','Powder-Brownlee','Hells Canyon','Lower Snake Asotin')) {
      ...
      TP_evaluate <- EvaluateTP_SRHC(new_data, 
                                     selectWQSTP = 0.07)
    } else if (tp_stn[j] %in% c('WR4000', 'WR5000', 'KL0010', '422820122032100', '422622122004000', '422719121571400', '422305121553800','422042121513100', '421401121480900')) {            
      TP_evaluate <- EvaluateTP_UKL_lake(new_data,
                                         annual_target=0.11, 
                                         spring_target=0.03)
      } else if (tp_stn[j] %in% c('10770', '11501000', '11502500', '21535', '422305121553803', '422622122004003', 'FWNF-085', 'FWNF-129','FWNF-130', 'FWNF-161', 'SR0040', 'SR0050', 'SR0060', 'SR0070', 'SR0080', 'SR0090', 'SR0140', 'WR1000', 'WR2000', 'WR3000', 'WR6000')) {
          TP_evaluate <- EvaluateTP_UKL_inflows(new_data,
                                                annual_target = 0.066)
        } else {
          TP_evaluate <- EvaluateTPWQS(new_data, selectWQSTP = 0)
    }
    ...
    } else if (tp_stn[j] %in% c('WR4000', 'WR5000', 'KL0010', '422820122032100', '422622122004000', '422719121571400',
                                '422305121553800','422042121513100', '421401121480900')) {
      
      b <- plot.TP.UKL(new_data = TP_evaluate,
                       annual_target = 0.11,
                       spring_target = 0.03,
                       sea_ken_table = results_seaken,
                       plot_trend = trend_logic,
                       parm = unique(TP_evaluate$Analyte))
    } else if (tp_stn[j] %in% c('10770', '11501000', '11502500', '21535', '422305121553803', '422622122004003', 'FWNF-085',
                                'FWNF-129','FWNF-130', 'FWNF-161', 'SR0040', 'SR0050', 'SR0060', 'SR0070', 'SR0080', 'SR0090',
                                'SR0140', 'WR1000', 'WR2000', 'WR3000', 'WR6000')) {
      b <- plot.TP(new_data=TP_evaluate,
                   sea_ken_table=results_seaken,
                   plot_trend = trend_logic,
                   selectWQSTP = 0.066,
...
conclusions.add.txt <- c("Grab samples may not provide a complete characterization of water quality for parameters that fluctuate on a daily basis                              (e.g., DO and pH). For this reason continuous data are preferred and should be considered when monitoring these parameters                            in the future.",
                         "This report is best used as a summary and statistical analysis of the status and trends in water quality data collected                               throughout the Klamath Headwaters Agricultural Water Quality Management Area. Interpretation of results will require                                  knowledge of local conditions known to affect the observed water quality conditions at individual sites.")
...
Oregon Department of Environmental Quality, 2002. Upper Klamath Lake Drainage Total Maximum Daily Load (TMDL) and Water Quality Management Plan (WQMP).
