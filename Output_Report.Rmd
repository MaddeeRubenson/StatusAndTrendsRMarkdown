...
if(length(DO_stns) > 0) {
  ...
  DO_all <- merge(DO_all,in_bentable, by.x = "Station_ID", by.y = "Station_ID")
  
  if(any(is.na(DO_all$DO_use) | is.na(DO_all$spwn_time))) {
    print("In order to assess against water quality standard, need to add Dissolved Oxygen beneficial uses and spawning time periods to stations.csv for stations:")
      print(unique(DO_all[(is.na(DO_all$DO_use) | is.na(DO_all$spwn_time)),c("Station_ID")]))
    break
  }
  ...
  for(j in 1: length(DO_stns)) {
    new_data <- DO_all[DO_all$Station_ID == DO_stns[j],]
