...
if(file.exists(file=paste0(Rdata_dir,"/",df_all_clean_file))) {
  ...
  if(any(grepl('OREGONDEQ', df.all$Station_ID))) {
    df.all$Station_ID <- gsub("[A-Z]", "", sub$Station_ID)
    df.all$Station_ID <- gsub("-", "", sub$Station_ID)
  } 
  ...
  # is a calculate value like Daily Max or 7DADM. This needs to be incorporate somehow.
